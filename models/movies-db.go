...
// All returns all movie and error, if any
func (m *DBModel) All(genre ...int) ([]*Movie, error) { // ...int ถ้าไม่ใส่ Ellipsis จะเป็นการ require parameter แล้ว  app.models.DB.All() จะ error
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
